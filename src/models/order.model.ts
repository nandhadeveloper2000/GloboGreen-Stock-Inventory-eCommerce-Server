...
export const ORDER_STATUS = ["PLACED", "CONFIRMED", "PACKED", "SHIPPED", "DELIVERED", "CANCELLED"] as const;
...
const AddressSnapshotSchema = new Schema(
  {
    label: { type: String, default: "" }, // your customer address uses free label string
...
OrderSchema.pre("save", function (next: mongoose.CallbackWithoutResultAndOptionalError) {
  if ((this as any).orderNo) return next();
  const ts = Date.now().toString().slice(-8);
  const rand = Math.floor(Math.random() * 9000 + 1000);
  (this as any).orderNo = `OD${ts}${rand}`;
  next();
});
